...
class DefinitionProvider {
    ...
    brsFileGetDefinition(file) {
        ...
        if (token.kind === TokenKind_1.TokenKind.StringLiteral) {
            if ((0, reflection_1.isImportStatement)(expression)) {
                const pkgPath = util_1.default.getPkgPathFromTarget(file.pkgPath, expression.filePath);
                const importedFile = this.event.program.getFile(pkgPath);
                if (importedFile) {
                    this.event.definitions.push(util_1.default.createLocation(vscode_uri_1.URI.file(importedFile.srcPath).toString(), util_1.default.createRange(1, 0, 1, 0)));
                    return;
                }
            }
